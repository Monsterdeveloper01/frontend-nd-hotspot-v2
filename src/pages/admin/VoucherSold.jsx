    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 className="text-xl font-bold text-zinc-200">Riwayat Voucher Terjual</h2>
                <div className="relative">
                    ...
                    />
                    <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500"></i>
                </div>
            </div>
                    ...
                    <table className="w-full text-left min-w-[900px]">
                        <thead>
                            <tr className="bg-zinc-900/50 border-b border-admin-border">
                                ...
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Paket</th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Harga</th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Digunakan</th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Selesai</th>
                        ...
                        <tbody className="divide-y divide-admin-border">
                            {loading ? (
                                <tr>
                                    <td colSpan="7" className="px-8 py-20 text-center text-zinc-500 font-bold italic">
                                        Loading history records...
                                    </td>
                                </tr>
                            ) : soldVouchers.length > 0 ? soldVouchers.map((v) => (
                                <tr key={v.id} className="hover:bg-zinc-900/50/50 transition-colors group">
                                    <td className="px-8 py-6">
                                        <div className="font-black text-zinc-100 text-lg tracking-widest uppercase leading-none">{v.code}</div>
                                        <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1 block">Credential Key</span>
                                    </td>
                                    ...
                                    <td className="px-8 py-6 font-black text-zinc-200 text-sm tracking-tight">{v.plan?.name}</td>
                                    <td className="px-8 py-6 font-black text-zinc-100 italic tracking-tighter">
                                        Rp {Number(v.price).toLocaleString('id-ID')}
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-zinc-100 uppercase">
                                    ...
                                    <td className="px-8 py-6">
                                        <span className="px-4 py-1.5 bg-zinc-800 text-zinc-500 rounded-xl text-[9px] font-black uppercase tracking-widest border border-admin-border">
                                            Archive
                                        </span>
                                    </td>
                                </tr>
