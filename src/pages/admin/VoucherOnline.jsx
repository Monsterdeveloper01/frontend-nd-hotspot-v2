    ...
    return (
        <div className="space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-2xl font-black text-admin-text tracking-tight uppercase leading-none">Voucher Online</h1>
                    <p className="text-admin-muted font-bold text-[10px] uppercase tracking-widest mt-2">Monitoring sesi aktif dan riwayat login hotspot</p>
                </div>
                <button 
                    onClick={fetchData} 
                    className="flex items-center gap-3 px-6 py-4 bg-admin-card border border-admin-border rounded-2xl font-black text-[10px] uppercase tracking-widest text-admin-text hover:bg-admin-base transition-all shadow-sm active:scale-95"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-admin-card p-6 rounded-[24px] border border-admin-border shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-admin-muted uppercase tracking-widest">Total Active</p>
                ...
                <div className="bg-admin-card p-6 rounded-[24px] border border-admin-border shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-admin-muted uppercase tracking-widest">Live Online</p>
                ...
                <div className="bg-admin-card p-6 rounded-[24px] border border-admin-border shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-admin-muted uppercase tracking-widest">Offline (Jeda)</p>
                        <h3 className="text-3xl font-black text-admin-muted tracking-tighter">{stats.offline}</h3>
                    </div>
                    <div className="w-14 h-14 bg-admin-base text-admin-muted rounded-2xl flex items-center justify-center border border-admin-border shadow-sm">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-admin-card p-4 rounded-[24px] border border-admin-border shadow-sm">
                <div className="w-full md:w-1/2 relative">
                    <input 
                        ...
                        className="w-full pl-6 pr-14 py-4 bg-admin-base border border-admin-border rounded-[20px] text-sm font-bold text-admin-text focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-admin-card transition-all placeholder:text-admin-muted"
                    />
                    <div className="absolute right-2 top-2 bottom-2 aspect-square flex items-center justify-center bg-slate-200 text-admin-muted rounded-2xl shadow-sm">
                ...
                <div className="w-full md:w-auto">
                    <select 
                        ...
                        className="w-full md:w-64 px-6 py-4 bg-admin-base border border-admin-border rounded-[20px] text-sm font-bold text-admin-text focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-admin-card transition-all cursor-pointer uppercase tracking-widest appearance-none"
            ...
            <div className="flex flex-col gap-10">
                ...
                <div className="w-full">
                    <div className="bg-admin-card rounded-2xl shadow-sm border border-admin-border overflow-hidden">
                        <div className="px-10 py-6 border-b border-admin-border bg-admin-base/50 flex items-center gap-4">
                            <div className="w-10 h-10 bg-emerald-500 text-admin-text rounded-xl flex items-center justify-center shadow-lg shadow-emerald-200">
                                <Icon name="wifi" className="w-5 h-5" />
                            </div>
                            <h2 className="text-sm font-black text-admin-text uppercase tracking-widest">Active Vouchers</h2>
                        </div>
                            ...
                            <table className="w-full text-left min-w-[1000px]">
                                <thead>
                                    <tr className="bg-admin-base/50 border-b border-admin-border">
                                        ...
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-admin-muted">Profil Paket</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-admin-muted">Waktu Pemakaian</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-admin-muted">Sisa Waktu</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-admin-muted">Device MAC</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {paginatedVouchers.length > 0 ? paginatedVouchers.map((v) => (
                                        <tr key={v.id} className="hover:bg-blue-50/30 transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black transition-colors ${v.is_online ? 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-admin-text' : 'bg-admin-base text-admin-muted group-hover:bg-admin-base0 group-hover:text-admin-text'}`}>
                                                        <Icon name="wifi" className="w-4 h-4" />
                                                    </div>
                                                    <div>
                                                        <span className="font-black text-admin-text text-lg tracking-widest uppercase block leading-none">{v.code}</span>
                                                        {v.is_online ? (
                                                            ...
                                                            <span className="text-[10px] text-admin-muted font-black uppercase tracking-widest mt-1 flex items-center gap-1.5">
                                            ...
                                            <td className="px-8 py-6">
                                                <span className="font-black text-admin-text text-sm uppercase tracking-tight">{v.plan?.name}</span>
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex flex-col gap-1">
                                                    <span className="text-[10px] font-bold text-admin-muted uppercase flex items-center gap-2">
                                                        <span className="w-12 inline-block text-admin-muted">Mulai:</span> 
                                                    ...
                                                    <span className="text-[10px] font-bold text-rose-500 uppercase flex items-center gap-2">
                                                        <span className="w-12 inline-block text-admin-muted">Habis:</span>
                                            ...
                                            <td className="px-8 py-6">
                                                <code className="text-[10px] font-black bg-admin-base px-3 py-1.5 rounded-lg text-admin-muted uppercase tracking-widest font-mono border border-admin-border">{v.mac_address || 'Unbound'}</code>
                                            </td>
                                            <td className="px-8 py-6 text-right">
                                                {v.is_online ? (
                                                    <button
                                                        onClick={() => handleKick(v.code)}
                                                        className="p-3 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-admin-text transition-all active:scale-95 shadow-sm border border-rose-100"
                                        ...
                                        <tr>
                                            <td colSpan="6" className="px-8 py-24 text-center text-admin-muted font-black italic uppercase tracking-[0.2em]">
                ...
                <div className="w-full">
                    <div className="bg-admin-card rounded-2xl p-8 shadow-sm border border-admin-border">
                        <div className="flex items-center gap-4 mb-8">
                            <div className="w-10 h-10 bg-slate-900 text-admin-text rounded-xl flex items-center justify-center">
                                <Icon name="history" className="w-5 h-5" />
                            </div>
                            <h3 className="text-sm font-black text-admin-text tracking-widest uppercase">Log Activity Terakhir</h3>
                        </div>
                            ...
                            {logs.length > 0 ? logs.map((log) => (
                                <div key={log.id} className="relative pl-6 border-l-2 border-admin-border py-2 group">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="font-black text-admin-text text-sm tracking-widest uppercase leading-none">{log.code}</span>
                                        <div className="flex items-center justify-between mt-3">
                                            <span className="text-[9px] font-black text-admin-muted uppercase tracking-widest">
